...
export class PresetRepository {
  ...
  findAll(): PresetEntity[] {
    const rows = this.db.prepare('SELECT * FROM presets ORDER BY updated_at DESC').all() as PresetRecord[];
    return rows.map(this.toEntity);
  }

  findById(id: number): PresetEntity | null {
    const row = this.db.prepare('SELECT * FROM presets WHERE id = ?').get(id) as PresetRecord | undefined;
  ...
  create(input: CreatePresetDto): PresetEntity {
    ...
    const result = this.db
      ...
      .run(
        ...
        state.osc1.waveform, state.osc1.octave, state.osc1.detune, state.osc1.volume,
        state.filter.enabled ? 1 : 0, state.filter.cutoff, state.filter.resonance, state.filter.slope, state.filter.envelope,
        state.ampAdsr.attack, state.ampAdsr.decay, state.ampAdsr.sustain, state.ampAdsr.release,
        state.arpeggiator.enabled ? 1 : 0, state.arpeggiator.rate,
        state.global.midiChannel, state.global.voices, state.global.multiCore ? 1 : 0
      );
  ...
  update(id: number, input: UpdatePresetDto): PresetEntity {
    ...
    this.db
      .prepare(`UPDATE presets SET 
        ...
        WHERE id = ?`)
      .run(
        next.name, next.description ?? null,
        next.state.osc1.waveform, next.state.osc1.octave, next.state.osc1.detune, next.state.osc1.volume,
        next.state.filter.enabled ? 1 : 0, next.state.filter.cutoff, next.state.filter.resonance, next.state.filter.slope, next.state.filter.envelope,
        next.state.ampAdsr.attack, next.state.ampAdsr.decay, next.state.ampAdsr.sustain, next.state.ampAdsr.release,
        next.state.arpeggiator.enabled ? 1 : 0, next.state.arpeggiator.rate,
        next.state.global.midiChannel, next.state.global.voices, next.state.global.multiCore ? 1 : 0,
        id
      );
